...
router.post('/add', cryptographyService.authorize, upload.array('pictures'), async (req, res) => {
    try {
        const { name, description, features, githubRepo, technologies, technologyTag, status, ytVideoId, score } = req.body;
        const pictures = req.files.map(file => file.filename);
        await projectService.addProject(name, description, JSON.parse(features), githubRepo, JSON.parse(technologies), technologyTag, status, pictures, ytVideoId, score);
        res.sendStatus(200);
    } catch(error) {
...
router.post('/edit', cryptographyService.authorize, upload.array('uploadedPictures'), async (req, res) => {
    try {
        const { _id, name, description, features, githubRepo, technologies, technologyTag, status, allPictures, deletedPictures, ytVideoId, score } = req.body;
